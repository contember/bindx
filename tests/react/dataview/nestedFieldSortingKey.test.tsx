// Regression test for https://github.com/contember/bindx/issues/68
//
...
import { SelectionScope, SchemaRegistry, EntityHandle } from '@contember/bindx'
...
describe('useSortingState — nested field refs', () => {
	...
	// The same guard has to hold for a LIVE row accessor, not just the collector
	// proxy: DataViewSortingTrigger accepts any FieldRef, so a nested ref taken
	// off a rendered row must produce the same dotted key.
	test('should toggle sorting for a nested ref taken off a live entity handle', () => {
		const { store, dispatcher } = createTestDispatcher()
		store.setEntityData('Project', 'p-1', { id: 'p-1', name: 'Alpha', stats: { id: 's-1', memberCount: 3 } }, true)

		const project = EntityHandle.create<Project>('p-1', 'Project', store, dispatcher, schemaRegistry)
		const liveNestedRef = project.stats.memberCount

		expect(extractFieldName(liveNestedRef)).toBe('stats.memberCount')
		...
		act(() => {
			result.current.setOrderBy(liveNestedRef, 'next')
		})

		expect(result.current.resolvedOrderBy).toEqual([{ stats: { memberCount: 'asc' } }])
		expect(result.current.directionOf(liveNestedRef)).toBe('asc')
	})
})
