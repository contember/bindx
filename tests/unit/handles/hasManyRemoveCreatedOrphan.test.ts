// Regression test for https://github.com/contember/bindx/issues/47
//
...
describe('HasManyListHandle remove() of a created entity', () => {
	...
	test('should not leave an orphan create in the store after add() then remove()', () => {
		...
		expect(handle.items.some(i => i.id === tempId)).toBe(true)
		...
		expect(handle.items.some(i => i.id === tempId)).toBe(false)

		// And it must no longer be reported as a create — otherwise a global
		// persistAll picks it up and tries to create the removed row. (The snapshot
		// may linger until the lazy sweep; reachability is what makes it harmless.)
